...
if (isset($_POST['fav_type'])) {
	...
	if ($favType == 'fav-colleges') {
		...
		echo $content;
				...
				$liked_community_discussions = get_liked_community_discussions($userId);
				$liked_discussions = get_liked_discussions($userId);
				if (!empty($liked_community_discussions)) {
					foreach ($liked_community_discussions as $key) {
						...
						if (!is_null($key['major_id'])) {
							...
							$content .= '</ul>';
							$content .= '<div class="forum-post-body"><a href="community-discussion.php?school_name=' .$urlCollegeName ;
							$content .= '&community_id=' . $key['community_id'] . '&c_discussion_id=' . $key['c_discussion_id'] . '"><p class="forum-title community-forum">' . nl2br($key['c_discussion_post']) . '</p></a></div>';							
						}
						$content .= '</li>';
					}
				}
				...
				$liked_events = get_liked_events($userId);
				if(!empty($liked_events)){
