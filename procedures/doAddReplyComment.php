...
if (isset($_POST['r-reply-post'])) {
	...
	$addReplyComment = add_reply_comments($dReplyId, $collegeId, $discussionId, $studentId, $rReplyPost);
	...
	$username = trim(filter_input(INPUT_POST,"username",FILTER_SANITIZE_STRING));
	if($communityId){
		$addReplyComment = add_c_reply_comments($dReplyId, $collegeId, $communityId, NULL, $discussionId, $studentId, $rReplyPost);
	}elseif($majorId){
		$addReplyComment = add_c_reply_comments($dReplyId, $collegeId, NULL, $majorId, $discussionId, $studentId, $rReplyPost);
	}
