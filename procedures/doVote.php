...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		$check = get_vote($discussionId,$userId);
		...
		if(!empty($check)){
			if ($action == "upvote-".$discussionId) {
				if ($check['vote'] == 1) {
					...
					update_vote($discussionId,$userId,'up');
					echo "add-upvote";
				}elseif($check['vote'] == -1){
					update_vote($discussionId,$userId,'up');
					echo "clear-upvote";
				...
				if ($check['vote'] == -1) {
					...
					update_vote($discussionId,$userId,'down');
					echo "add-downvote";
				}elseif($check['vote'] == 1){
					update_vote($discussionId,$userId,'down');
					echo "clear-downvote";
			...
			if ($action == "upvote-".$discussionId) {
				$upvote = new_vote($discussionId,$userId,'up');	
				echo "new-upvote";
			}elseif($action == "downvote-".$discussionId){
				$upvote = new_vote($discussionId,$userId,'down');
				echo "new-downvote";
			}
		}
